...
const initial = {
  currentMatchImage: imagesList[0],
  activeTab: 'FRUIT',
  score: 0,
  timer: 60,
...
class App extends Component {
  ...
  changeImage = id => {
    const {currentMatchImage, timer} = this.state

    const randomNumber = Math.ceil(Math.random() * 29)

    if (currentMatchImage.id === id && timer !== 0) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({currentMatchImage: imagesList[randomNumber]})
    } else {
      ...
      <div className="bgContainer">
        ...
        {isGameInProgress ? (
          <div className="gameContainer">
            <img
              src={currentMatchImage.imageUrl}
              alt="match"
              ...
              {tabsList.map(eachTab => (
                <li key={eachTab.tabId}>
                  <button
                    type="button"
                    className={`tabItem ${
                      activeTab === eachTab.tabId ? 'highlight' : ''
                    }`}
            ...
            <ul className="thumbnailsContainer">
              {filteredList.map(eachThumbnail => (
                <li className="listItem" key={eachThumbnail.id}>
                  <button
                    type="button"
                    onClick={() => this.changeImage(eachThumbnail.id)}
                    className="button"
                  >
                    <img
          ...
          <div className="gameResultContainer">
            ...
            <h1 className="finalScore">{score}</h1>
            <button
              type="button"
